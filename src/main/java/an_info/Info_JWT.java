...
//		JWT - Signed Tokens - It is NOT ENCRYPTED!
...
//	JWT data protection
// • JWT does NOT encrypt data in any way.
// • JWT protected against tampering and assures it is from trusted source.
// • JWT data is Base 64 encoded and digitally signed but not encrypted.
// • Base 64 encoding is for data structure change.
// • Signing using public/private key pairs, certifies that only the party holding the private key is the one that signed it.
...
//   allowing the user to access routes, services, and resources that are permitted with that token. Single Sign On is a feature that widely
...
//-This can be, in certain cases, a stateless authorization mechanism. The server's protected routes will check for a valid JWT
// in the Authorization header, and if it's present, the user will be allowed to access protected resources. If the JWT contains
// the necessary data, the need to query the database for certain operations may be reduced, though this may not always be the case.
//
...
//    Fe, a typical OpenID Connect compliant web application will go through the /oauth/authorize endpoint using the authorization code flow.
...
//-Security-wise, SWT can only be symmetrically signed by a shared secret using the HMAC algorithm. However, JWT and SAML tokens
// can use a public/private key pair in the form of a X.509 certificate for signing. Signing XML with XML Digital Signature without
// introducing obscure security holes is very difficult when compared to the simplicity of signing JSON.
//
...
//
//-Regarding usage, JWT is used at Internet scale. This highlights the ease of client-side processing
// of the JSON Web token on multiple platforms, especially mobile.




public class Info_JWT {}